(function () {
  ...
  function buildBanner(existingConsent) {
    ...
    title.textContent = isReopen ? 'Hantera cookie-inställningar' : s.banner_title;
    inner.appendChild(title);

    var text = document.createElement('p');
    text.className = 'ccm-text';
    text.textContent = isReopen
      ? 'Här kan du ändra eller dra tillbaka ditt samtycke.'
      : s.banner_text;
    if (!isReopen && s.privacy_policy_url) {
      ...
      policyLink.textContent = ' Läs vår integritetspolicy.';
    ...
    if (isReopen && existingConsent) {
      ...
      if (existingConsent.timestamp) {
        ...
        var dateStr = d.toLocaleDateString('sv-SE', { day: 'numeric', month: 'short', year: 'numeric' })
          + ' - ' + d.toLocaleTimeString('sv-SE');
        var dateLine = document.createElement('p');
        dateLine.innerHTML = '<strong>Samtyckesdatum:</strong> ' + dateStr;
        consentInfo.appendChild(dateLine);
      }
      if (existingConsent.uuid) {
        var idLine = document.createElement('p');
        idLine.innerHTML = '<strong>Ditt samtyckes-ID:</strong> ' + existingConsent.uuid;
    ...
    if (isDNTEnabled() && !isReopen) {
      ...
      dntNotice.textContent = 'Din webbläsare har Do Not Track aktiverat. Analys- och marknadsföringscookies är automatiskt avaktiverade.';
    ...
    if (isReopen) {
      ...
      withdrawBtn.textContent = 'Dra tillbaka samtycke';
      ...
      toggleDetails.textContent = 'Göm detaljer';
      toggleDetails.addEventListener('click', function () {
        var isVisible = details.style.display !== 'none';
        details.style.display = isVisible ? 'none' : 'block';
        saveBtn.style.display = isVisible ? 'none' : 'inline-block';
        toggleDetails.textContent = isVisible ? 'Visa detaljer' : 'Göm detaljer';
      });
